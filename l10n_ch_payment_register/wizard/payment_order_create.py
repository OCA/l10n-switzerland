...
class payment_order_create(osv.osv_memory):
    ...
    def create_payment(self, cr, uid, ids, context=None):
        order_obj = self.pool.get('payment.register')
        line_obj = self.pool.get('account.move.line')
        payment_obj = self.pool.get('payment.register.line')
        if context is None:
            context = {}
...
#                     'name': line.name,
                    ...
                    'date': date_to_pay,
                    'currency': (line.invoice_id and line.invoice_id.currency_id.id) or line.journal_id.currency.id or line.journal_id.company_id.currency_id.id,
                }, context=context)
        return {'type': 'ir.actions.act_window_close'}
...
#         if line_ids:
#             self.write(cr, uid, ids, {'entries': [(6,0,line_ids)]}, context)
        context = dict(context, line_ids=line_ids)
        model_data_ids = mod_obj.search(cr, uid,[('model', '=', 'ir.ui.view'), ('name', '=', 'view_payment_order_create_lines')], context=context)
