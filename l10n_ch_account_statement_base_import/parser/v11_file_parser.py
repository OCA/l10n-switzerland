...
class V11FileParser(BankStatementImportParser):
    ...
    def __init__(self, parser_name, ftype='v11', **kwargs):
        ...
        if ftype not in ('v11', 'esr' ,'bvr'):
            raise except_osv(_('User Error'),
                             _('Invalid file type %s. Please use v11, esr or bvr') % ftype)
    ...
    def _pre(self, *args, **kwargs):
        """
        Check which kind of record type is the file, by looking at the total line.
        """
    ...
    def _parse_type4(self):
        """
        Parse ESR record of type 4. This method is not implemented. Feel free to inherit from this class in your module in order to implement it.
        """
        return NotImplementedError

    def _validate(self, *args, **kwargs):
        """
        We check our results against the total line for record of type 3. Inherit this method to add validation for record type 4.
        """
        if self.record_type == 3:
            if not len(self.result_row_list) == int(self.lines[-1][51:63]):
                raise except_osv(_('Invalid data'),
                                     _('The number of read transactions doesn\'t match the total record in file'))
