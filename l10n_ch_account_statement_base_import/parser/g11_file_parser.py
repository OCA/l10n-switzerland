...
class G11FileParser(BankStatementImportParser):
    ...
    def __init__(self, parser_name, ftype='g11', **kwargs):
        ...
        if ftype != 'g11':
            raise except_osv(_('User Error'),
                             _('Invalid file type %s. Please use g11') % ftype)
    ...
    def _pre(self, *args, **kwargs):
        ...
        if total_line[0:3] == '097':
            self.balance_end = (float(total_line[45:57]) / 100) - (float(total_line[101:113]) / 100)
            self.commission = float(total_line[131:142]) / 100 # not used
            self.amount_fail = float(total_line[77:89]) / 100 # not used
            self.transactions = int(total_line[57:69]) + int(total_line[89:101]) + int(total_line[113:125])
            # Store the currency
            self.currency = total_line[128:131]
        else:
            raise osv.except_osv(_("Wrong formatting"),
             _("The total record could not be read. Please provide a valid g11 file."))
        return True
    ...
    def _parse(self, *args, **kwargs):
        ...
        for line in self.lines[:-1]:
            ...
            test = line[108:116]
            date = time.strftime('%Y-%m-%d', time.strptime(line[108:116], '%Y%m%d'))
            commission = float(line[141:147]) / 100 # not used
            label = ''
            ...
            if line[0:3] == '084':
                ...
                if reject_code == '02':
                    ...
                    # Failed transactions. Get the error reason and put it on the OBI field.
                    label = reject_reason[reject_code] + '\n' + _("Amount to debit was %s %f") % (currency, amount)
                    amount = 0.0
    ...
    def _validate(self, *args, **kwargs):
        ...
        if len(self.result_row_list) != self.transactions:
            raise osv.except_osv(_("Invalid number of transactions"),
             _("The number of transactions read is not the same as the one provided in the total record."))
