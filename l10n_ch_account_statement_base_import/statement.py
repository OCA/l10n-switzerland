...
class AccountStatementProfil(Model):
    ...
    def _get_import_type_selection(self, cr, uid, context=None):
        ...
        res.extend([('raiffeisen_csvparser', 'Parser for Raiffeisen (CH-FR) statements'),
                    ('raiffeisen_details_csvparser', 'Parser for detailed Raiffeisen (CH-FR) statements'),
                    ...
                    ('g11_fileparser', 'Parser for Postfinance BVR DD type 2'),
                    ])
        return res
    ...
    def prepare_statement_vals(self, cr, uid, profile_id, result_row_list, parser, context):
        ...
        if hasattr(parser, 'currency'):
            ...
            if not (parser.currency == profile.journal_id.currency.name or parser.currency == profile.journal_id.company_id.currency_id.name):
                raise osv.except_osv(_("Wrong currency"),
                _("The transactions you are importing are not in the same currency than the selected journal !"))
                
        return super(AccountStatementProfil, self).prepare_statement_vals(cr, uid, profile_id, result_row_list, parser, context)
    ...
    def _statement_import(self, cr, uid, ids, prof, parser, file_stream, ftype="csv", context=None):
        ...
        return super(AccountStatementProfil, self)._statement_import(cr, uid, ids, prof, parser, file_stream, ftype, context)
     
    def _write_extra_statement_lines(self, cr, uid, parser, result_row_list, profile, statement_id, context):
        """ For Postfinance XML parser, we join the attached documents to the statement. """
            ...
            for filename in tfile.getnames():
                if filename.endswith('.png'):
                    attachment_data = {
                        ...
                        'datas_fname': "%s.%s" % (datetime.datetime.now().date(), filename[-4:]),
                        'res_model': 'account.bank.statement',
                        'res_id': statement_id,
                    }
                    self.pool.get('ir.attachment').create(cr, uid, attachment_data, context=context)